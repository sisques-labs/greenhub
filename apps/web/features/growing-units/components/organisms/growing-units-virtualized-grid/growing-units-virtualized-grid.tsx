...
export function GrowingUnitsVirtualizedGrid({
	growingUnits,
	onAddClick,
}: GrowingUnitsVirtualizedGridProps) {
	...
	const items = [...growingUnits, { id: 'add-card', isAddCard: true }];
						...
						>
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 h-full">
								{rowItems.map((item: any) =>
									item.isAddCard ? (
										<GrowingUnitAddCard key="add-card" onClick={onAddClick} />
									) : (
										<GrowingUnitCard key={item.id} growingUnit={item} />
									),
								)}
							</div>
						</div>
