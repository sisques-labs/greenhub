...
// Alias for backward compatibility with SDK naming
export type AuthUserProfileResponse = UserProfile;

// Helper to transform API response to UserProfile
export function transformUserProfile(
	apiResponse: UserProfileApiResponse,
): UserProfile {
	return {
		...apiResponse,
		lastLoginAt: apiResponse.lastLoginAt
			? new Date(apiResponse.lastLoginAt)
			: null,
