...
describe('LocationTypeormMapper', () => {
	...
	beforeEach(() => {
		...
		mapper = new LocationTypeormMapper(mockLocationAggregateBuilder);
	});
	...
	describe('toDomainEntity', () => {
		it('should convert TypeORM entity to domain aggregate with all properties', () => {
			...
			const locationAggregate = locationAggregateBuilder
				...
				.withDescription(new LocationDescriptionValueObject(
					'North-facing room with good sunlight',
				))
				.build();

			mockLocationAggregateBuilder.build.mockReturnValue(locationAggregate);

			const result = mapper.toDomainEntity(typeormEntity);

			expect(result).toBe(locationAggregate);
			expect(mockLocationAggregateBuilder.fromPrimitives).toHaveBeenCalledWith({
				id: locationId,
				name: 'Living Room',
