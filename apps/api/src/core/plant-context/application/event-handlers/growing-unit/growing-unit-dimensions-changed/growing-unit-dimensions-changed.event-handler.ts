...
/**
 * Event handler for GrowingUnitDimensionsChangedEvent.
 *
 * @remarks
 * This handler updates the growing unit view model when the dimensions are changed,
 * ensuring the read model is synchronized with the write model.
 */
@EventsHandler(GrowingUnitDimensionsChangedEvent)
export class GrowingUnitDimensionsChangedEventHandler
  implements IEventHandler<GrowingUnitDimensionsChangedEvent>
{
  private readonly logger = new Logger(
    GrowingUnitDimensionsChangedEventHandler.name,
  );
  ...
  /**
   * Handles the GrowingUnitDimensionsChangedEvent event by updating the growing unit view model.
   *
   * @param event - The GrowingUnitDimensionsChangedEvent event to handle
   */
  async handle(event: GrowingUnitDimensionsChangedEvent) {
    this.logger.log(
      `Handling growing unit dimensions changed event: ${event.aggregateId}`,
    );

    this.logger.debug(
      `Growing unit dimensions changed event data: ${JSON.stringify(event.data)}`,
    );
