import { PlantSpeciesFindAllQuery } from '@/core/plant-species-context/application/queries/plant-species/plant-species-find-all/plant-species-find-all.query';
import { PlantSpeciesFindAllQueryHandler } from '@/core/plant-species-context/application/queries/plant-species/plant-species-find-all/plant-species-find-all.query-handler';
...
describe('PlantSpeciesFindAllQueryHandler', () => {
	let handler: PlantSpeciesFindAllQueryHandler;
	...
	beforeEach(() => {
		...
		handler = new PlantSpeciesFindAllQueryHandler(
			mockPlantSpeciesReadRepository,
		);
	});
	...
	describe('execute', () => {
		...
		it('should return all plant species view models', async () => {
			const query = new PlantSpeciesFindAllQuery();
			mockPlantSpeciesReadRepository.findAll.mockResolvedValue([mockViewModel]);

			const result = await handler.execute(query);

			expect(result).toEqual([mockViewModel]);
			expect(mockPlantSpeciesReadRepository.findAll).toHaveBeenCalledTimes(1);
		});

		it('should return empty array when no plant species exist', async () => {
			const query = new PlantSpeciesFindAllQuery();
			mockPlantSpeciesReadRepository.findAll.mockResolvedValue([]);
