...
import {
  ...
  PlantReadRepository,
} from '@/features/plant-context/plants/domain/repositories/plant-read/plant-read.repository';
...
import { PlantStatusChangedEventHandler } from './plant-status-changed.event-handler';

describe('PlantStatusChangedEventHandler', () => {
  let handler: PlantStatusChangedEventHandler;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<PlantStatusChangedEventHandler>(
      PlantStatusChangedEventHandler,
    );
  });
  ...
  describe('handle', () => {
    it('should update and save plant view model status when event is handled', async () => {
      const plantId = '123e4567-e89b-12d3-a456-426614174000';
      const event = new PlantStatusChangedEvent(
        {
          aggregateId: plantId,
