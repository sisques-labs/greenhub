...
import { LogOut, Home, Sprout, LayoutGrid, Settings } from 'lucide-react';
  ...
  return (
    <Sidebar {...props} variant="sidebar" className="border-r border-sidebar-border/50">
            ...
            <SidebarMenuButton asChild size="lg" className="gap-3">
              <a href={data.header.url || '#'}>
                ...
                <div className="flex flex-col items-start gap-0.5">
                  <span className="font-bold text-base leading-none">{data.header.appName}</span>
                  <span className="text-xs text-sidebar-foreground/60 font-normal">Dashboard</span>
                </div>
              </a>
            </SidebarMenuButton>
          </SidebarMenuItem>
            ...
            <SidebarGroupContent>
              <SidebarMenu>
                {group.items.map((item) => {
                  const IconComponent = item.icon || iconMap[item.title.toLowerCase()];
                  return (
      ...
      <SidebarFooter className="border-t border-sidebar-border/50 bg-gradient-to-t from-sidebar/50 to-sidebar/30 backdrop-blur-sm">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton asChild size="lg" className="gap-3 hover:bg-sidebar-accent/50">
              <a href={data.footer.profileUrl} className="flex items-center gap-3">
                ...
                <div className="flex flex-col items-start gap-0.5 min-w-0 flex-1">
                  <span className="font-semibold text-sm truncate w-full">{data.footer.name}</span>
                  <span className="text-xs text-sidebar-foreground/60 truncate w-full">View profile</span>
                </div>
              </a>
            </SidebarMenuButton>
            {onLogout && (
