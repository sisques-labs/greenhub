...
export class GraphQLClient {
  ...
  async request<T>(options: GraphQLRequestOptions): Promise<T> {
    ...
    // Get token dynamically if getToken function is provided, otherwise use stored token
    let token: string | null | undefined = this.accessToken;
    if (this.getToken) {
      const dynamicToken = await this.getToken();
      token = dynamicToken || this.accessToken;
    }
    ...
    if (response.status === 401 && this.refreshToken) {
      try {
        ...
        // Get token dynamically if getToken function is provided
        let retryToken: string | null | undefined = this.accessToken;
        if (this.getToken) {
          const dynamicToken = await this.getToken();
          retryToken = dynamicToken || this.accessToken;
        }
